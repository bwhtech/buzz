...
`
		...
		(t._x_transitioning = {
			...
			requestAnimationFrame(() => {
				...
				let s =
					...
					a =
						Number(
							getComputedStyle(t).transitionDelay.replace(/,.*/, "").replace("s", "")
						) * 1e3;
				s === 0 &&
					(s = Number(getComputedStyle(t).animationDuration.replace("s", "")) * 1e3),
					m(() => {
						e.before();
					}),
					(n = !0),
	...
	function yt(t) {
		return (
			t.type === "checkbox" || t.localName === "ui-checkbox" || t.localName === "ui-switch"
		);
	}
	function jt(t) {
	...
	function eo(t, e) {
		...
		return (
			...
			!(
				i.length > 0 &&
				i.filter((s) => ((s === "cmd" || s === "super") && (s = "meta"), t[`${s}Key`]))
					.length === i.length &&
