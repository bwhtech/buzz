...
frappe.ui.form.on("Event Booking", {
	refresh(frm) {
		frm.set_query("ticket_type", "attendees", (doc) => {
			return {
				filters: {
					event: doc.event,
				},
		...
		if (
			...
			if (frm.doc.refund_status !== "Refunded") {
				frm.add_custom_button(__("Issue Refund"), () => showRefundDialog(frm), __("Refunds"))
			}
...
async function showRefundDialog(frm) {
	...
	if (refundable <= 0) {
		frappe.msgprint({
			...
			message: __("Every rupee on this booking is already refunded or waiting on the gateway."),
			indicator: "orange",
		})
		return
	}
		...
		fields: [
			{
				...
				options: tickets.map((ticket) => ({
					label: `${ticket.attendee} — ${format_currency(ticket.amount, frm.doc.currency)}`,
					value: ticket.ticket,
				})),
				on_change: () => {
